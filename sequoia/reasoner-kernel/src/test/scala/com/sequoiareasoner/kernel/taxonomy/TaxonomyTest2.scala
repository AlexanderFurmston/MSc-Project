...
class TaxonomyTest2 extends FunSuite {
  ...
  // THIS IS ALWAYS a taxonimy of SequoiaOWLClasses, in this test, this is why we can input bottom entity and top entity.
  private[this] def computeTaxonomy(input: Set[(IRI, IRI)]): (Set[Set[IRI]], Set[(IRI, IRI)]) = {
    ...
    val taxonomy = Taxonomy(map,Some(IRI.owlNothing),Some(IRI.owlThing))
