...
/** The type of sets of taxonomy nodes.
  ...
  */
abstract class TaxonomyNodeSet[X <: OWLObject] extends CollectionMakeString {
  ...
  /** Tests if the specified element is contained in this set.
    ...
    */
  def contains(elem: TaxonomyNode[X]): Boolean
  /** Returns `true` if the specified a predicate holds for at least one node in this set.
    ...
    */
  def exists(p: TaxonomyNode[X] => Boolean): Boolean
  /** Returns `true` if the specified predicate holds for all nodes in this set.
    ...
    */
  def forall(p: TaxonomyNode[X] => Boolean): Boolean
  /** Applies a function `f` to all elements of this set.
    ...
    */
  def foreach[U](f: TaxonomyNode[X] => U): Unit
  def iterator: Iterator[TaxonomyNode[X]]
  ...
  /** For efficiency, these sets need to be mutable. However, they are unmodifiable outside of this package.
    ...
    */
  protected[taxonomy] def add(elem: TaxonomyNode[X]): Boolean
}

protected[taxonomy] object TaxonomyNodeSetImpl {

  def apply[X <: OWLObject](elem: TaxonomyNode[X]): TaxonomyNodeSetImpl[X] = {
    val set = new TaxonomyNodeSetImpl[X]
    set.add(elem)
    set
  }
}

protected[taxonomy] final class TaxonomyNodeSetImpl[X <: OWLObject] extends TaxonomyNodeSet[X] {
  private[this] val mutableSet = new collection.mutable.HashSet[TaxonomyNode[X]]
  ...
  override def contains(elem: TaxonomyNode[X]): Boolean = mutableSet.contains(elem)
  override def exists(p: TaxonomyNode[X] => Boolean): Boolean = mutableSet.exists(p)
  override def forall(p: TaxonomyNode[X] => Boolean): Boolean = mutableSet.forall(p)
  override def foreach[U](f: TaxonomyNode[X] => U): Unit = mutableSet.foreach(f)
  override def iterator: Iterator[TaxonomyNode[X]] = mutableSet.iterator
  ...
  override def equals(o: Any): Boolean = o match {
    case that: TaxonomyNodeSet[X] if this.size == that.size =>
      mutableSet.forall(that.contains)
    case _ => false
  }
  override protected[taxonomy] def add(elem: TaxonomyNode[X]): Boolean = mutableSet.add(elem)
}
