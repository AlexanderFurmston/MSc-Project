...
//  def ::(that: IndexedSequence[A]): IndexedSequence[A] = {
//    val firstSequence = this
//    new IndexedSequence[A]{
//      override def length = firstSequence.length + that.length
//      override def apply(index: Int): A = {
//        if (index < firstSequence.length) firstSequence(index)
//        else that(index - firstSequence.length)
//      }
//      override def isEmpty: Boolean = false
//      override def nonEmpty: Boolean = true
//      override def iterator: Iterator[A] = firstSequence.iterator ++ that.iterator
//      override def forall(p: A => Boolean): Boolean = firstSequence.forall(p) && that.forall(p)
//      override def exists(p: A => Boolean): Boolean = firstSequence.exists(p) || that.exists(p)
//      override def contains(o: A): Boolean = firstSequence.contains(o) || that.contains(o)
//      override def foreach[U](f: A => U): Unit = { firstSequence.foreach(f); that.foreach(f) }
//      override def addString(b: StringBuilder, start: String, sep: String, end: String): StringBuilder = {
//        var builder = b.append(start)
//        for (elem <- firstSequence.iterator) builder = builder.append(elem).append(sep)
//        for (elem <- that.iterator) builder = builder.append(elem).append(sep)
//        builder.append(end)
//      }
//      override def hashCode: Int = 31*(31 + firstSequence.hashCode + that.hashCode)
//      override def equals(o: Any): Boolean = o match {
//        case sequence: IndexedSequence[_] => (sequence.length == (firstSequence.length + that.length)) &&
//          (0 until (firstSequence.length + that.length)).forall {
//          x => { if (x < firstSequence.length) firstSequence(x) else that(x - firstSequence.length) } == sequence(x)}
//        case _ => false
//      }
//    }
//  }
