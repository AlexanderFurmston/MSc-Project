...
import com.sequoiareasoner.kernel.index.{ImmutableSet, MutableIndexedSequence}
...
/** A context clause index that additionally maintains subsumption information. This class is intended to be used by
  ...
  *
  */
final class RootContextClauseIndex(importedFacts: MutableIndexedSequence[IRI], core: ImmutableSet[Predicate] = ImmutableSet.empty[Predicate]) extends ContextClauseIndex {

  override val rootContext = true
  ...
  override def add(c: ContextClause): Unit = {
    ...
    if (c.body.length == 0) {
      if (c.head.length == 1) {
        c.head(0) match {
          case Concept(iri, CentralVariable) if iri.isImported =>  {
//            println("adding: ")
//            println(iri)
//            println("To context with core " + core)
            importedFacts += iri
          }
          case _ => // Do nothing
        }
